...
try:
    import neo4j
except ImportError, neo4j_import_exception:
    neo4j = types.ModuleType('neo4j', 'Fake module')

    class Neo4JGraphWrapper(GraphWrapper):
        def __init__(self):
            raise neo4j_import_exception
else:

    db = neo4j.GraphDatabase()

    class Neo4JWrapper(GraphWrapper):
        def __init__(self, graph):
            """
            Creates an implementation of Graph backed with igraph.Graph
            @param graph: the backing graph
            @type graph: neo4j.GraphDatabase
            """
            self.graph = graph

        def add_node(self, identifier, agent):
            self.graph.node(value=identifier)
